    ...
    for dataset_name in dataset_name_list:
        ...
        transform_test = transforms.Compose([
            ...
            transforms.Normalize(
                mean=transformed_norm_weights["OSDataset_transformed"]["mean"],
                std=transformed_norm_weights["OSDataset_transformed"]["std"])])
        ...
        #elif dataset_name == "CRC_transformed":
        #    dataset = TransformedDataset(root="/ssd2/HistoKT/datasets/Full/CRC_transformed", split="test", transform=transform_test)
        else:
        ...
        tune = os.path.join(path_to_dataset_cp, "deep_tuning")
        for file in os.listdir(tune):
            rate = os.path.join(tune, file)
            for file2 in os.listdir(rate):
                if ".pth" in file2 and "best_" in file2:
                    path_to_pth = os.path.join(rate, file2)
                    print(path_to_pth)
                    if path_to_output is not None:
                        path_to_out_data = os.path.join(path_to_output, dataset_name)
                        path_to_out_data = os.path.join(path_to_out_data, "AdamP/deep_tuning")
                        ...
                        test_results(path_to_pth, test_dataloader, dataset_nam)
    return


if __name__ == "__main__":
    checkpoint = "/ssd2/HistoKT/results/post_training_without_color_aug_ImageNet/OSDataset_transformed_norm_OSDataset_transformed_color_aug_None_ImageNet"
    root = "/ssd2/HistoKT/datasets"
    output = "/ssd2/HistoKT/test/ImageNet_output/OSDataset_transformed_norm_OSDataset_transformed_color_aug_None_ImageNet_output"  # None if same as the checkpoint dir

    dataset_name_list = ["CRC_transformed"]#["AJ-Lymph_transformed", "BACH_transformed", "BCSS_transformed", "CRC_transformed", "GlaS_transformed", "MHIST_transformed", "PCam_transformed"]

...
def run_fine_tune_combined(root, CC=False, node="cedar", account="def-msh"):
    runscripts = defaultdict(list)
    # account = "def-plato"
    # account = "def-msh"
    ...
    for weight_folder in weight_folders:
        for dataset in datasets:
            ...
            pretrained_models = os.listdir(os.path.join(root, weight_folder))
...
early_stop_patience: 10 # epoch window to consider when deciding whether to stop"""
                    ...
                    runscripts[weight_folder].append(
    ...
    for dataset in datasets:
        for weight_folder in weight_folders:
            ...
            if CC:
                ...
                env_name = "env"
                data_dir = f"/ssd{gpu_start + 1}/users/mhosseini/datasets/"
            for wt_type in ["preSVD", "postSVD"]:
                pretrained_models = os.listdir(os.path.join(root, dataset, weight_folder, wt_type))
                for pretrained_model in pretrained_models:
                    normalization_all = "target_domain" if "ImageNet" not in weight_folder else "ImageNet"
                    pretrained_model_name = os.path.basename(pretrained_model).split(".")[0] + wt_type
...
early_stop_patience: 10 # epoch window to consider when deciding whether to stop"""
                        ...
                        runscripts[dataset][weight_folder].append(
                            f"run{dataset}-{optimizer}-lr-{learning_rate}-{pretrained_model_name}-norm-{normalization_all}-{color_aug}.sh")
...
"""
    ...
    if CC:
        ...
        for dataset in datasets:
            for weight_folder in weight_folders:
                with open(f"runlambda_weight_transfer_{weight_folder}_{dataset}.sh", "w") as outfile:
                    outlines = [f"bash {script}\n" for script in runscripts[dataset][weight_folder]]
...
if __name__ == "__main__":
    ...
    run_tune_combined_SVD(root_dir, CC=False)